...
async function run() {
  ...
  if (!dirExists(skillsDest)) {
    log.warn('No skills/ folder found in the current directory.');
  ...
  log.warn('Any local changes inside skills/ will be overwritten.');
  ...
  try {
    ...
    log.line('');
    log.info('To see what changed:  git diff skills/');
    log.line('');
  } finally {
    rl.close();
  }
