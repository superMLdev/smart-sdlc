...
/**
 * Returns the subset of PERSONAS whose sequential prerequisites are satisfied.
 * Personas with no prerequisites (product, modernization) are always eligible.
 */
function getEligiblePersonas(config) {
  const artifacts = (config && config.artifacts) || {};
  return PERSONAS.filter(p => {
    if (!p.prerequisites || p.prerequisites.length === 0) return true;
    return p.prerequisites.every(flag => !!artifacts[flag]);
  });
}

async function runMeeting(rl, projectRoot) {
  log.banner();
  log.line('🎉 Party Mode — structured multi-persona collaboration session.');
  log.line('   Bring multiple AI perspectives to one meeting with governed agenda and outputs.');
  log.line('');
  ...
  // ── Template or custom? ────────────────────────────────────────────────────
  log.section('Party Mode — meeting type');
  ...
  if (meetingMode === 'template') {
    ...
    // Filter personas by eligibility (sequential phase gates)
    const eligibleKeys      = new Set(getEligiblePersonas(config).map(p => p.key));
    const eligibleDefaults  = template.defaultPersonas.filter(k => eligibleKeys.has(k));
    const lockedDefaults    = template.defaultPersonas.filter(k => !eligibleKeys.has(k));
    const eligibleOptionals = template.optionalPersonas.filter(k => eligibleKeys.has(k));
    preSelectedKeys = [...eligibleDefaults];
    ...
    allRelevant.forEach(k => {
      ...
      if (!eligibleKeys.has(k)) {
        log.line(`    ✗ locked   ${name} (${p.role}) — phase not yet reached`);
      } else {
        const tag = template.defaultPersonas.includes(k) ? '● default' : '○ optional';
        log.line(`    ${tag}  ${name} (${p.role})`);
      }
    });
    if (lockedDefaults.length > 0) {
      log.line('');
      log.warn('Some default personas are locked (their phase has not started yet):');
      lockedDefaults.forEach(k => {
        const p      = PERSONAS.find(p => p.key === k);
        const reason = (p.prerequisiteLabels || []).join('; ') || 'prerequisites not met';
        log.line(`      ✗  ${p.name} (${p.role}): ${reason}`);
      });
    }
    log.line('');

    // Allow user to add optional personas (eligible only)
    if (eligibleOptionals.length > 0) {
      const optionalChoices = eligibleOptionals.map(k => {
        const p    = PERSONAS.find(p => p.key === k);
        const name = (config && config[p.configKey]) || p.name;
  ...
  if (meetingMode === 'custom') {
    const eligibleForCustom = getEligiblePersonas(config);
    const lockedForCustom   = PERSONAS.filter(p => !eligibleForCustom.find(e => e.key === p.key));

    if (lockedForCustom.length > 0) {
      log.line('');
      log.line('  Personas not yet available (phase not reached):');
      lockedForCustom.forEach(p => {
        const name = (config && config[p.configKey]) || p.name;
        log.line(`    \u2717  ${name} (${p.role})`);
      });
      log.line('');
    }

    const personaChoices = eligibleForCustom.map(p => {
