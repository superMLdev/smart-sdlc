...
async function runMeeting(rl, projectRoot) {
  ...
  if (!config) {
    log.warn('No _superml/config.yml found. Run `npx @supermldev/agentic-sdlc init` first.');
  ...
  if (useCopilot) {
    ...
    for (const p of selectedPersonas) {
      log.item(`#file:skills/${p.skills[0]}/SKILL.md   ← ${p.resolvedName} (${p.role})`);
    }
    log.line('');
    ...
    for (const p of selectedPersonas) {
      log.item(`"Also load the skill at skills/${p.skills[0]}/SKILL.md"`);
    }
    log.line('');
    log.line('Then say:');
    log.line('');
...
function buildMeetingDoc({ topic, date, projectName, docsPath, personas, useCopilot }) {
  ...
  for (const p of personas) {
    ...
    p.skills.slice(0, 2).forEach(s =>
      lines.push(useCopilot ? `- \`#file:skills/${s}/SKILL.md\`` : `- \`skills/${s}/SKILL.md\``)
    );
    lines.push('');
  }
  ...
  lines.push('---');
  lines.push('');
  lines.push('*Generated by Agentic SDLC — `npx @supermldev/agentic-sdlc meeting` | Superml.dev & superml.org by crazyaiml*');
