 ...
 */
function checkPersonaGate(config, personaKey) {
...
async function runPersona(rl, projectRoot) {
  ...
  const supermlDir     = path.join(projectRoot, '_superml');
  const personaDest    = path.join(supermlDir, 'persona.yml');
  let existingPersonaKey = null;

  if (fileExists(personaDest)) {
    const existingYml  = readYml(personaDest);
    existingPersonaKey = existingYml && existingYml.primary_persona;
    log.warn('_superml/persona.yml already exists.');
    if (existingPersonaKey) {
      const cur = PERSONA_MAP[existingPersonaKey];
      if (cur) log.line(`  Currently active as: ${cur.name} \u2014 ${cur.role}`);
    }
    const overwrite = await confirm(rl, 'Re-install persona with new answers?', false);
    if (!overwrite) {
      log.info('Keeping installed persona config.');
      log.line('');
      if (existingYml) {
        const existingTools = parseYmlList(existingYml.ai_tools) || (existingYml.use_github_copilot !== 'false' ? ['github_copilot'] : []);
        showNextSteps(config, existingPersonaKey, existingTools);
      }
      return;
    }
  }
  ...
  // ── Persona switch warning ────────────────────────────────────────────────
  if (existingPersonaKey && existingPersonaKey !== primaryPersona) {
    const from = PERSONA_MAP[existingPersonaKey];
    const to   = PERSONA_MAP[primaryPersona];
    log.line('');
    log.warn(`Changing installed persona: ${from ? `${from.name} (${from.role})` : existingPersonaKey} → ${to ? `${to.name} (${to.role})` : primaryPersona}`);
    log.line('  Changing your installed role may leave previous work incomplete.');
    log.line('');
    const confirmSwitch = await confirm(rl, 'Continue with persona change?', false);
    if (!confirmSwitch) {
      log.info('Persona change cancelled. Keeping your installed persona.');
      log.line('');
      process.exit(0);
    }
  }
  ...
  const gate = checkPersonaGate(config, primaryPersona);
  if (gate.length > 0) {
