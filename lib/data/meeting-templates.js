...
const MEETING_TEMPLATES = [
...

  {
    ...
    optionalPersonas: ['product', 'team_lead', 'qa'],
    agenda: [
...

  {
    ...
    optionalPersonas: ['product', 'team_lead', 'release'],
    agenda: [
...

  {
    ...
    defaultPersonas:  ['architect', 'developer'],
    optionalPersonas: ['product', 'modernization'],
    agenda: [
...

  {
    ...
    label: 'Release Readiness Review  — gate-check before shipping',
    title: 'Release Readiness Review',
    purpose:
      'Gate-check the release before it ships. QA confirms all tests pass and bugs are triaged, ' +
      'the developer confirms no outstanding regressions, and the Release Manager produces the ' +
      'final go / no-go decision with a documented deployment plan.',
    defaultPersonas:  ['qa', 'release', 'developer'],
    optionalPersonas: ['team_lead', 'product'],
    agenda: [
      'QA: test execution summary — pass/fail counts, open defects, severity breakdown',
      'Developer: confirm no outstanding regressions or deferred bugs',
      'Release Manager: deployment plan, rollback procedure, feature flags',
      'Risk assessment: any open items that could block the release?',
      'Go / No-Go decision — explicit, documented, with rationale',
      'Release Manager: assign on-call owner and post-release monitoring window',
    ],
    output: {
      type:         'release-readiness',
      pathTemplate: '{docsPath}/meetings/release-readiness-{slug}.md',
    },
    topicPrompt: 'Release version or feature name (e.g. "v2.4.0" or "Payments MVP")?',
    discussionStarters: [
      '"{persona0}, walk us through the test results. What\'s the pass rate?"',
      '"{persona1}, are there any deferred bugs we should know about?"',
      '"{persona2}, is the deployment plan solid? What does rollback look like?"',
      '"What\'s the single biggest risk if we ship today?"',
      '"Go or No-Go? State your answer and one-sentence rationale — {persona0}, start."',
    ],
  },
];
